...
33. 응
34. api-client.ts:44  POST http://localhost:3000/api/proxy/auth/login 500 (Internal Server Error)
35. Failed to load resource: the server responded with a status of 503 (Service Unavailable)
백엔드랑 확실하게 연동하란 말이야. 뭐가 더 필요하면 말을 하고 
36. # 개요
...
그 분이 준건 이거야
37. api 명세서대로 확실히 연동한거 맞지? api/proxy/auth/login:1 
...
api-client.ts:44 
 POST http://localhost:3000/api/proxy/auth/login 503 (Service Unavailable)
Promise.then		
apiRequest	@	api-client.ts:44
login	@	auth-service.ts:19
